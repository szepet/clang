...
public:
  ...
  bool ImportDeclParts(NamedDecl *D, DeclContext *&DC, DeclContext *&LexicalDC,
  ...
  TemplateArgumentLoc
  ImportTemplateArgumentLoc(const TemplateArgumentLoc &TALoc, bool &Error);
  ...
  void ImportArray(IIter Ibegin, IIter Iend, OIter Obegin) {
    ...
    std::transform(Ibegin, Iend, Obegin, [&ImporterRef](ItemT From) -> ItemT {
...
struct StructuralEquivalenceContext {
  ...
  llvm::DenseMap<std::pair<Decl *, ASTContext *>, Decl *> TentativeEquivalences;
  ...
  StructuralEquivalenceContext(
      ...
      : C1(C1), C2(C2), NonEquivalentDecls(NonEquivalentDecls),
        StrictTypeSpelling(StrictTypeSpelling), Complain(true),
...
public:
  DiagnosticBuilder Diag1(SourceLocation Loc, unsigned DiagID) {
    ...
    LastDiagFromC2 = false;
    C1.getDiagnostics().setWarningsAsErrors(true);
  ...
  DiagnosticBuilder Diag2(SourceLocation Loc, unsigned DiagID) {
    ...
    LastDiagFromC2 = true;
    C2.getDiagnostics().setWarningsAsErrors(true);
...
static bool IsStructurallyEquivalent(const IdentifierInfo *Name1,
  ...
  if (Name1->getName().str() == "share" && !Name1->getName().equals(Name2->getName()))
  {
    assert(false);
  }
...
static bool IsStructurallyEquivalent(StructuralEquivalenceContext &Context,
  ...
  for (RecordDecl::field_iterator Field1 = D1->field_begin(),
    ...
    /*if (!IsStructurallyEquivalent(Context, *Field1, *Field2)) {
      if (D1->getNameAsString() == "Curl_easy")
        i++;
    }*/
    if(Field1->getNameAsString() == "share" && D1->getNameAsString() == "Curl_easy")
    {
      i++;
    }
    if(!IsStructurallyEquivalent(Context, *Field1, *Field2))
...
static bool IsStructurallyEquivalent(StructuralEquivalenceContext &Context,
  ...
  Decl *&EquivToD1 = Context.TentativeEquivalences[std::make_pair(
      D1->getCanonicalDecl(), &Context.C2)];
  /*int i = 0;
  if (auto newD = dyn_cast<NamedDecl>(D1)) {
    if (newD->getNameAsString() == "share" && EquivToD1 &&
...
bool StructuralEquivalenceContext::Finish() {
  while (!DeclsToCheck.empty()) {
    ...
    int i = 0;
    Decl *D2 = TentativeEquivalences[std::make_pair(D1,&C2)];
    assert(D2 && "Unrecorded tentative equivalence?");
    NamedDecl *newD = dyn_cast<NamedDecl>(D1);
    if (newD) {
      // llvm::errs() << "YOLOMOLO " << newD->getNameAsString() << "\n";
      if (newD->getNameAsString() == "share") {
        i++;
      }
    }
    if (i == -1)
    ...
    if (RecordDecl *Record1 = dyn_cast<RecordDecl>(D1)) {
      if (RecordDecl *Record2 = dyn_cast<RecordDecl>(D2)) {
        ...
        /*if (!::IsStructurallyEquivalent(Name1, Name2) ||
          ...
          if (newD->getNameAsString() == "Curl_easy") {
            i++;
          }
        }*/
        if (newD->getNameAsString() == "Curl_easy") {
...
QualType ASTNodeImporter::VisitConstantArrayType(const ConstantArrayType *T) {
  ...
  return Importer.getToContext().getConstantArrayType(
      ToElementType, T->getSize(), T->getSizeModifier(),
...
ASTNodeImporter::VisitIncompleteArrayType(const IncompleteArrayType *T) {
  ...
  return Importer.getToContext().getIncompleteArrayType(
      ToElementType, T->getSizeModifier(), T->getIndexTypeCVRQualifiers());
}
...
QualType ASTNodeImporter::VisitVectorType(const VectorType *T) {
  ...
  return Importer.getToContext().getVectorType(
      ToElementType, T->getNumElements(), T->getVectorKind());
}
...
QualType ASTNodeImporter::VisitUnaryTransformType(const UnaryTransformType *T) {
  ...
  return Importer.getToContext().getUnaryTransformType(
      ToBaseType, ToUnderlyingType, T->getUTTKind());
}
...
ASTNodeImporter::VisitInjectedClassNameType(const InjectedClassNameType *T) {
  ...
  return QualType(new (Importer.getToContext(), TypeAlignment)
                      InjectedClassNameType(D, InjType),
                  0);
}
...
QualType ASTNodeImporter::VisitEnumType(const EnumType *T) {
  EnumDecl *ToDecl = dyn_cast_or_null<EnumDecl>(Importer.Import(T->getDecl()));
  if (!ToDecl)
    return QualType();
...
QualType ASTNodeImporter::VisitTemplateSpecializationType(
  ...
  return Importer.getToContext().getTemplateSpecializationType(
      ToTemplate, ToTemplateArgs, ToCanonType);
}
...
QualType ASTNodeImporter::VisitElaboratedType(const ElaboratedType *T) {
  ...
  return Importer.getToContext().getElaboratedType(T->getKeyword(), ToQualifier,
                                                   ToNamedType);
}
...
QualType ASTNodeImporter::VisitObjCObjectType(const ObjCObjectType *T) {
  ...
  return Importer.getToContext().getObjCObjectType(
      ToBaseType, TypeArgs, Protocols, T->isKindOfTypeAsWritten());
}
  ...
  if (CXXRecordDecl *ToCXX = dyn_cast<CXXRecordDecl>(To)) {
    ...
    for (const auto &Base1 : FromCXX->bases()) {
      ...
      Bases.push_back(new (Importer.getToContext()) CXXBaseSpecifier(
          Importer.Import(Base1.getSourceRange()), Base1.isVirtual(),
          Base1.isBaseOfClass(), Base1.getAccessSpecifierAsWritten(),
          Importer.Import(Base1.getTypeSourceInfo()), EllipsisLoc));
    }
    if (!Bases.empty())
...
bool ASTNodeImporter::ImportDefinition(EnumDecl *From, EnumDecl *To,
  ...
  To->completeDefinition(T, ToPromotionType, From->getNumPositiveBits(),
                         From->getNumNegativeBits());
  return false;
}

TemplateParameterList *
ASTNodeImporter::ImportTemplateParameterList(TemplateParameterList *Params) {
  ...
  return TemplateParameterList::Create(
      Importer.getToContext(), Importer.Import(Params->getTemplateLoc()),
      Importer.Import(Params->getLAngleLoc()), ToParams,
      Importer.Import(Params->getRAngleLoc()), ToRequiresClause);
}
...
bool ASTNodeImporter::ImportTemplateArguments(
...
Decl *ASTNodeImporter::VisitAccessSpecDecl(AccessSpecDecl *D) {
  ...
  AccessSpecDecl *accessSpecDecl = AccessSpecDecl::Create(
      Importer.getToContext(), D->getAccess(), DC, Loc, ColonLoc);
...
Decl *ASTNodeImporter::VisitNamespaceDecl(NamespaceDecl *D) {
  ...
  if (!ToNamespace) {
    ToNamespace = NamespaceDecl::Create(
        Importer.getToContext(), DC, D->isInline(),
...
Decl *ASTNodeImporter::VisitTypedefNameDecl(TypedefNameDecl *D, bool IsAlias) {
  ...
  if (IsAlias)
    ...
    ToTypedef = TypedefDecl::Create(Importer.getToContext(), DC, StartL, Loc,
                                    Name.getAsIdentifierInfo(), TInfo);
...
Decl *ASTNodeImporter::VisitLabelDecl(LabelDecl *D) {
  ...
  LabelDecl *ToLabel =
      D->isGnuLocal() ? LabelDecl::Create(Importer.getToContext(), DC,
                                          Importer.Import(D->getLocation()),
                                          Name.getAsIdentifierInfo(),
                                          Importer.Import(D->getLocStart()))
                      : LabelDecl::Create(Importer.getToContext(), DC,
                                          Importer.Import(D->getLocation()),
                                          Name.getAsIdentifierInfo());
...
Decl *ASTNodeImporter::VisitEnumDecl(EnumDecl *D) {
  ...
  EnumDecl *D2 = EnumDecl::Create(
      Importer.getToContext(), DC, Importer.Import(D->getLocStart()), Loc,
...
Decl *ASTNodeImporter::VisitRecordDecl(RecordDecl *D) {
  // If this record has a definition in the translation unit we're coming
  // from,
    ...
    for (unsigned I = 0, N = FoundDecls.size(); I != N; ++I) {
      ...
      if (RecordDecl *FoundRecord = dyn_cast<RecordDecl>(Found)) {
        if (D->isAnonymousStructOrUnion() &&
          ...
          // sure
          // they occur in the same location in the context records.
          if (Optional<unsigned> Index1 = findUntaggedStructOrUnionIndex(D)) {
        ...
        if (RecordDecl *FoundDef = FoundRecord->getDefinition()) {
          ...
          // If one or both can be completed from external storage then try
          // one
          ...
          if (FoundRecord->hasExternalLexicalStorage() &&
            ...
            FoundRecord->getASTContext().getExternalSource()->CompleteType(
                FoundRecord);
          if (D->hasExternalLexicalStorage())
            D->getASTContext().getExternalSource()->CompleteType(D);

          if (FoundRecord->isCompleteDefinition() &&
              D->isCompleteDefinition() && !IsStructuralMatch(D, FoundRecord))
            continue;
  ...
  if (!D2) {
    ...
    if (CXXRecordDecl *DCXX = llvm::dyn_cast<CXXRecordDecl>(D)) {
      if (DCXX->isLambda()) {
        ...
        D2CXX = CXXRecordDecl::CreateLambda(
            Importer.getToContext(), DC, TInfo, Loc, DCXX->isDependentLambda(),
        ...
        D2CXX =
            CXXRecordDecl::Create(Importer.getToContext(), D->getTagKind(), DC,
...
Decl *ASTNodeImporter::VisitEnumConstantDecl(EnumConstantDecl *D) {
  ...
  EnumConstantDecl *ToEnumerator = EnumConstantDecl::Create(
      Importer.getToContext(), cast<EnumDecl>(DC), Loc,
...
Decl *ASTNodeImporter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  if (ToD)
    return ToD;
  llvm::errs() << "visitFnDecl: " << Name.getAsString() << "\n";
  bool b = (Name.getAsString() == "Curl_share_unlock");
  int i = 1;
    ...
    for (unsigned I = 0, N = FoundDecls.size(); I != N; ++I) {
      ...
      if (auto *FoundFunction = dyn_cast<FunctionDecl>(FoundDecls[I])) {
        if (FoundFunction->hasExternalFormalLinkage() &&
          ...
          // llvm::errs() << "LOLOLOLO   " << Name.getAsString() << "\n\n";
  ...
  if (auto *FromConstructor = dyn_cast<CXXConstructorDecl>(D)) {
    ToFunction = CXXConstructorDecl::Create(
    ...
    ToFunction = CXXDestructorDecl::Create(
        Importer.getToContext(), cast<CXXRecordDecl>(DC), InnerLocStart,
  ...
  if (FoundWithoutBody) {
    auto *Recent =
        const_cast<FunctionDecl *>(FoundWithoutBody->getMostRecentDecl());
...
Decl *ASTNodeImporter::VisitFieldDecl(FieldDecl *D) {
  ...
  FieldDecl *ToField = FieldDecl::Create(
      Importer.getToContext(), DC, Importer.Import(D->getInnerLocStart()), Loc,
      Name.getAsIdentifierInfo(), T, TInfo, BitWidth, D->isMutable(),
...
Decl *ASTNodeImporter::VisitFriendDecl(FriendDecl *D) {
  ...
  FriendDecl *FrD = FriendDecl::Create(
      Importer.getToContext(), DC, Importer.Import(D->getLocation()), ToFU,
...
Decl *ASTNodeImporter::VisitObjCIvarDecl(ObjCIvarDecl *D) {
  ...
  ObjCIvarDecl *ToIvar = ObjCIvarDecl::Create(
      Importer.getToContext(), cast<ObjCContainerDecl>(DC),
...
Decl *ASTNodeImporter::VisitVarDecl(VarDecl *D) {
  ...
  TypeSourceInfo *TInfo = Importer.Import(D->getTypeSourceInfo());
  VarDecl *ToVar = VarDecl::Create(
      Importer.getToContext(), DC, Importer.Import(D->getInnerLocStart()), Loc,
      Name.getAsIdentifierInfo(), T, TInfo, D->getStorageClass());
...
Decl *ASTNodeImporter::VisitImplicitParamDecl(ImplicitParamDecl *D) {
  ...
  ImplicitParamDecl *ToParm = ImplicitParamDecl::Create(
      Importer.getToContext(), DC, Loc, Name.getAsIdentifierInfo(), T);
...
Decl *ASTNodeImporter::VisitParmVarDecl(ParmVarDecl *D) {
  ...
  // Create the imported parameter.
  TypeSourceInfo *TInfo = Importer.Import(D->getTypeSourceInfo());
  ParmVarDecl *ToParm = ParmVarDecl::Create(
      Importer.getToContext(), DC, Importer.Import(D->getInnerLocStart()), Loc,
      Name.getAsIdentifierInfo(), T, TInfo, D->getStorageClass(),
...
Decl *ASTNodeImporter::VisitObjCTypeParamDecl(ObjCTypeParamDecl *D) {
  ...
  ObjCTypeParamDecl *Result = ObjCTypeParamDecl::Create(
      Importer.getToContext(), DC, D->getVariance(),
...
Decl *ASTNodeImporter::VisitObjCCategoryDecl(ObjCCategoryDecl *D) {
  ...
  if (!ToCategory) {
    ToCategory = ObjCCategoryDecl::Create(
        Importer.getToContext(), DC, Importer.Import(D->getAtStartLoc()), Loc,
        Importer.Import(D->getCategoryNameLoc()), Name.getAsIdentifierInfo(),
        ToInterface,
    ...
    ToCategory->setTypeParamList(
        ImportObjCTypeParamList(D->getTypeParamList()));
  ...
  if (D->getImplementation()) {
    ObjCCategoryImplDecl *Impl = cast_or_null<ObjCCategoryImplDecl>(
...
Decl *ASTNodeImporter::VisitLinkageSpecDecl(LinkageSpecDecl *D) {
  ...
  LinkageSpecDecl *ToLinkageSpec =
      LinkageSpecDecl::Create(Importer.getToContext(), DC, ExternLoc, LangLoc,
...
ASTNodeImporter::ImportObjCTypeParamList(ObjCTypeParamList *list) {
  ...
  for (auto fromTypeParam : *list) {
    auto toTypeParam =
        cast_or_null<ObjCTypeParamDecl>(Importer.Import(fromTypeParam));
    if (!toTypeParam)
      return nullptr;
  ...
  return ObjCTypeParamList::create(
      Importer.getToContext(), Importer.Import(list->getLAngleLoc()),
      toTypeParams, Importer.Import(list->getRAngleLoc()));
}
...
Decl *ASTNodeImporter::VisitObjCInterfaceDecl(ObjCInterfaceDecl *D) {
  ...
  if (!ToIface) {
    ToIface = ObjCInterfaceDecl::Create(
        Importer.getToContext(), DC, Importer.Import(D->getAtStartLoc()),
        Name.getAsIdentifierInfo(),
...
Decl *ASTNodeImporter::VisitObjCCategoryImplDecl(ObjCCategoryImplDecl *D) {
  ObjCCategoryDecl *Category =
      cast_or_null<ObjCCategoryDecl>(Importer.Import(D->getCategoryDecl()));
  if (!Category)
    return nullptr;
  ...
  if (!ToImpl) {
    ...
    ToImpl = ObjCCategoryImplDecl::Create(
        Importer.getToContext(), DC, Importer.Import(D->getIdentifier()),
...
Decl *ASTNodeImporter::VisitObjCImplementationDecl(ObjCImplementationDecl *D) {
  ...
  if (!Impl) {
    ...
    // now.
    Impl = ObjCImplementationDecl::Create(
        Importer.getToContext(), Importer.ImportContext(D->getDeclContext()),
...
Decl *ASTNodeImporter::VisitObjCPropertyDecl(ObjCPropertyDecl *D) {
  ...
  ObjCPropertyDecl *ToProperty = ObjCPropertyDecl::Create(
      Importer.getToContext(), DC, Loc, Name.getAsIdentifierInfo(),
...
Decl *ASTNodeImporter::VisitObjCPropertyImplDecl(ObjCPropertyImplDecl *D) {
  ObjCPropertyDecl *Property =
      cast_or_null<ObjCPropertyDecl>(Importer.Import(D->getPropertyDecl()));
  if (!Property)
    return nullptr;
  ...
  if (!ToImpl) {
    ToImpl = ObjCPropertyImplDecl::Create(
        Importer.getToContext(), DC, Importer.Import(D->getLocStart()),
    ...
    // Check that we have the same kind of property implementation
    // (@synthesize
    // vs. @dynamic).
    if (D->getPropertyImplementation() != ToImpl->getPropertyImplementation()) {
      Importer.ToDiag(ToImpl->getLocation(),
                      diag::err_odr_objc_property_impl_kind_inconsistent)
          << Property->getDeclName() << (ToImpl->getPropertyImplementation() ==
...
Decl *ASTNodeImporter::VisitTemplateTypeParmDecl(TemplateTypeParmDecl *D) {
  ...
  return TemplateTypeParmDecl::Create(
      Importer.getToContext(), Importer.getToContext().getTranslationUnitDecl(),
...
ASTNodeImporter::VisitNonTypeTemplateParmDecl(NonTypeTemplateParmDecl *D) {
  ...
  return NonTypeTemplateParmDecl::Create(
      Importer.getToContext(), Importer.getToContext().getTranslationUnitDecl(),
...
ASTNodeImporter::VisitTemplateTemplateParmDecl(TemplateTemplateParmDecl *D) {
  ...
  return TemplateTemplateParmDecl::Create(
      Importer.getToContext(), Importer.getToContext().getTranslationUnitDecl(),
      Loc, D->getDepth(), D->getPosition(), D->isParameterPack(),
      Name.getAsIdentifierInfo(), TemplateParams);
}
...
Decl *ASTNodeImporter::VisitClassTemplateDecl(ClassTemplateDecl *D) {
  ...
  // We may already have a template of the same name; try to find and match
  // it.
  if (!DC->isFunctionOrMethod()) {
    SmallVector<NamedDecl *, 4> ConflictingDecls;
    SmallVector<NamedDecl *, 2> FoundDecls;
    ...
    for (unsigned I = 0, N = FoundDecls.size(); I != N; ++I) {
      ...
      if (ClassTemplateDecl *FoundTemplate =
        ...
        if (IsStructuralMatch(D, FoundTemplate)) {
          // The class templates structurally match; call it the same
          // template.
  ...
  // Create the declaration that is being templated.
  CXXRecordDecl *D2Templated =
      cast_or_null<CXXRecordDecl>(Importer.Import(DTemplated));
  if (!D2Templated)
    return nullptr;
  ...
  ClassTemplateDecl *D2 = ClassTemplateDecl::Create(
...
Decl *ASTNodeImporter::VisitClassTemplateSpecializationDecl(
  ...
  ClassTemplateDecl *ClassTemplate = cast_or_null<ClassTemplateDecl>(
  ...
  if (D2) {
    ...
    D2 = ClassTemplateSpecializationDecl::Create(
        Importer.getToContext(), D->getTagKind(), DC, StartLoc, IdLoc,
        ClassTemplate, TemplateArgs,
        /*PrevDecl=*/nullptr);
...
Decl *ASTNodeImporter::VisitVarTemplateDecl(VarTemplateDecl *D) {
  ...
  // Import the major distinguishing characteristics of this variable
  // template.
  DeclContext *DC, *LexicalDC;
  DeclarationName Name;
  SourceLocation Loc;
...
DeclGroupRef ASTNodeImporter::ImportDeclGroup(DeclGroupRef DG) {
  ...
  std::transform(DG.begin(), DG.end(), ToDecls.begin(),
                 [&_Importer](Decl *D)
                     -> Decl *{ return _Importer.Import(D); });
...
Stmt *ASTNodeImporter::VisitGCCAsmStmt(GCCAsmStmt *S) {
  ...
  StringLiteral *AsmStr =
      cast_or_null<StringLiteral>(Importer.Import(S->getAsmString()));
  if (!AsmStr)
    return nullptr;

  return new (Importer.getToContext()) GCCAsmStmt(
      Importer.getToContext(), Importer.Import(S->getAsmLoc()), S->isSimple(),
      S->isVolatile(), S->getNumOutputs(), S->getNumInputs(), Names.data(),
      Constraints.data(), Exprs.data(), AsmStr, S->getNumClobbers(),
      Clobbers.data(), Importer.Import(S->getRParenLoc()));
}
...
Stmt *ASTNodeImporter::VisitNullStmt(NullStmt *S) {
  ...
  return new (Importer.getToContext())
      NullStmt(ToSemiLoc, S->hasLeadingEmptyMacro());
}
...
Stmt *ASTNodeImporter::VisitCompoundStmt(CompoundStmt *S) {
  ...
  return new (Importer.getToContext())
      CompoundStmt(Importer.getToContext(), ToStmts, ToLBraceLoc, ToRBraceLoc);
}
...
Stmt *ASTNodeImporter::VisitAttributedStmt(AttributedStmt *S) {
  ...
  std::transform(FromAttrs.begin(), FromAttrs.end(), ToAttrs.begin(),
                 [&_ToContext](const Attr *A)
                     -> const Attr *{ return A->clone(_ToContext); });
  ...
  if (!ToSubStmt && S->getSubStmt())
    return nullptr;
  return AttributedStmt::Create(Importer.getToContext(), ToAttrLoc, ToAttrs,
                                ToSubStmt);
}
...
Stmt *ASTNodeImporter::VisitIfStmt(IfStmt *S) {
  ...
  return new (Importer.getToContext()) IfStmt(
      Importer.getToContext(), ToIfLoc, S->isConstexpr(), ToInit,
...
Stmt *ASTNodeImporter::VisitSwitchStmt(SwitchStmt *S) {
  ...
  SwitchStmt *ToStmt = new (Importer.getToContext()) SwitchStmt(
      Importer.getToContext(), ToInit, ToConditionVariable, ToCondition);
...
Stmt *ASTNodeImporter::VisitWhileStmt(WhileStmt *S) {
  ...
  return new (Importer.getToContext())
      WhileStmt(Importer.getToContext(), ToConditionVariable, ToCondition,
                ToBody, ToWhileLoc);
}
...
Stmt *ASTNodeImporter::VisitDoStmt(DoStmt *S) {
  ...
  return new (Importer.getToContext())
      DoStmt(ToBody, ToCondition, ToDoLoc, ToWhileLoc, ToRParenLoc);
}
...
Stmt *ASTNodeImporter::VisitForStmt(ForStmt *S) {
  ...
  SourceLocation ToRParenLoc = Importer.Import(S->getRParenLoc());
  return new (Importer.getToContext())
      ForStmt(Importer.getToContext(), ToInit, ToCondition, ToConditionVariable,
...
Stmt *ASTNodeImporter::VisitReturnStmt(ReturnStmt *S) {
  ...
  if (!ToNRVOCandidate && NRVOCandidate)
    return nullptr;
  return new (Importer.getToContext())
      ReturnStmt(ToRetLoc, ToRetExpr, ToNRVOCandidate);
}
...
Stmt *ASTNodeImporter::VisitCXXForRangeStmt(CXXForRangeStmt *S) {
  ...
  SourceLocation ToRParenLoc = Importer.Import(S->getRParenLoc());
  return new (Importer.getToContext())
      CXXForRangeStmt(ToRange, ToBegin, ToEnd, ToCond, ToInc, ToLoopVar, ToBody,
...
Stmt *ASTNodeImporter::VisitObjCForCollectionStmt(ObjCForCollectionStmt *S) {
  ...
  SourceLocation ToRParenLoc = Importer.Import(S->getRParenLoc());
  return new (Importer.getToContext())
      ObjCForCollectionStmt(ToElem, ToCollect, ToBody, ToForLoc, ToRParenLoc);
}
...
Stmt *ASTNodeImporter::VisitObjCAtCatchStmt(ObjCAtCatchStmt *S) {
  ...
  if (!ToBody && S->getCatchBody())
    return nullptr;
  return new (Importer.getToContext())
      ObjCAtCatchStmt(ToAtCatchLoc, ToRParenLoc, ToExceptionDecl, ToBody);
}
...
Stmt *ASTNodeImporter::VisitObjCAtFinallyStmt(ObjCAtFinallyStmt *S) {
  ...
  if (!ToAtFinallyStmt && S->getFinallyBody())
    return nullptr;
  return new (Importer.getToContext())
      ObjCAtFinallyStmt(ToAtFinallyLoc, ToAtFinallyStmt);
}
...
Stmt *ASTNodeImporter::VisitObjCAtTryStmt(ObjCAtTryStmt *S) {
  ...
  if (!ToAtFinallyStmt && S->getFinallyStmt())
    return nullptr;
  return ObjCAtTryStmt::Create(Importer.getToContext(), ToAtTryLoc, ToAtTryStmt,
...
ASTNodeImporter::VisitObjCAutoreleasePoolStmt(ObjCAutoreleasePoolStmt *S) {
  ...
  if (!ToSubStmt && S->getSubStmt())
    return nullptr;
  return new (Importer.getToContext())
      ObjCAutoreleasePoolStmt(ToAtLoc, ToSubStmt);
}
...
Expr *ASTNodeImporter::VisitVAArgExpr(VAArgExpr *E) {
  ...
  return new (Importer.getToContext())
      VAArgExpr(Importer.Import(E->getBuiltinLoc()), SubExpr, TInfo,
...
Expr *ASTNodeImporter::VisitGNUNullExpr(GNUNullExpr *E) {
  ...
  return new (Importer.getToContext())
      GNUNullExpr(T, Importer.Import(E->getLocStart()));
}
...
Expr *ASTNodeImporter::VisitPredefinedExpr(PredefinedExpr *E) {
  ...
  StringLiteral *SL =
      cast_or_null<StringLiteral>(Importer.Import(E->getFunctionName()));
...
Expr *ASTNodeImporter::VisitDeclRefExpr(DeclRefExpr *E) {
  ...
  DeclRefExpr *DRE = DeclRefExpr::Create(
      ...
      Importer.Import(E->getLocation()), T, E->getValueKind(), FoundD, ResInfo);
...
ASTNodeImporter::ImportDesignator(const Designator &D) {
  ...
  return Designator(D.getFirstExprIndex(), Importer.Import(D.getLBracketLoc()),
...
Expr *ASTNodeImporter::VisitDesignatedInitExpr(DesignatedInitExpr *DIE) {
  ...
  llvm::transform(DIE->designators(), Designators.begin(),
                  [this](const Designator &D)
                      -> Designator { return ImportDesignator(D); });
  ...
  return DesignatedInitExpr::Create(
      Importer.getToContext(), Designators, IndexExprs,
      Importer.Import(DIE->getEqualOrColonLoc()), DIE->usesGNUSyntax(), Init);
}
...
Expr *ASTNodeImporter::VisitIntegerLiteral(IntegerLiteral *E) {
  ...
  return IntegerLiteral::Create(Importer.getToContext(), E->getValue(), T,
...
Expr *ASTNodeImporter::VisitCharacterLiteral(CharacterLiteral *E) {
  ...
  return new (Importer.getToContext()) CharacterLiteral(
      E->getValue(), E->getKind(), T, Importer.Import(E->getLocation()));
}
...
Expr *ASTNodeImporter::VisitStringLiteral(StringLiteral *E) {
  ...
  return StringLiteral::Create(Importer.getToContext(), E->getBytes(),
                               E->getKind(), E->isPascal(), T, Locations.data(),
                               Locations.size());
}
...
Expr *ASTNodeImporter::VisitCompoundLiteralExpr(CompoundLiteralExpr *E) {
  ...
  return new (Importer.getToContext())
      CompoundLiteralExpr(Importer.Import(E->getLParenLoc()), TInfo, T,
                          E->getValueKind(), Init, E->isFileScope());
}
...
Expr *ASTNodeImporter::VisitAtomicExpr(AtomicExpr *E) {
  ...
  if (ImportArrayChecked(E->getSubExprs(),
                         E->getSubExprs() + E->getNumSubExprs(), Exprs.begin()))
    return nullptr;
...
Expr *ASTNodeImporter::VisitAddrLabelExpr(AddrLabelExpr *E) {
  ...
  return new (Importer.getToContext())
      AddrLabelExpr(Importer.Import(E->getAmpAmpLoc()),
                    Importer.Import(E->getLabelLoc()), ToLabel, T);
}
...
Expr *ASTNodeImporter::VisitParenExpr(ParenExpr *E) {
  ...
  return new (Importer.getToContext())
      ParenExpr(Importer.Import(E->getLParen()),
                Importer.Import(E->getRParen()), SubExpr);
}
...
Expr *ASTNodeImporter::VisitParenListExpr(ParenListExpr *E) {
  ...
  return new (Importer.getToContext())
      ParenListExpr(Importer.getToContext(), Importer.Import(E->getLParenLoc()),
...
Expr *ASTNodeImporter::VisitStmtExpr(StmtExpr *E) {
  ...
  CompoundStmt *ToSubStmt =
      cast_or_null<CompoundStmt>(Importer.Import(E->getSubStmt()));
  if (!ToSubStmt && E->getSubStmt())
    return nullptr;

  return new (Importer.getToContext())
      StmtExpr(ToSubStmt, T, Importer.Import(E->getLParenLoc()),
               Importer.Import(E->getRParenLoc()));
}
...
Expr *ASTNodeImporter::VisitUnaryOperator(UnaryOperator *E) {
  ...
  return new (Importer.getToContext())
      UnaryOperator(SubExpr, E->getOpcode(), T, E->getValueKind(),
                    E->getObjectKind(), Importer.Import(E->getOperatorLoc()));
}
...
ASTNodeImporter::VisitUnaryExprOrTypeTraitExpr(UnaryExprOrTypeTraitExpr *E) {
  ...
  if (E->isArgumentType()) {
    ...
    return new (Importer.getToContext()) UnaryExprOrTypeTraitExpr(
        E->getKind(), TInfo, ResultType, Importer.Import(E->getOperatorLoc()),
        Importer.Import(E->getRParenLoc()));
  }
  ...
  return new (Importer.getToContext()) UnaryExprOrTypeTraitExpr(
      E->getKind(), SubExpr, ResultType, Importer.Import(E->getOperatorLoc()),
      Importer.Import(E->getRParenLoc()));
}
...
Expr *ASTNodeImporter::VisitBinaryOperator(BinaryOperator *E) {
  ...
  return new (Importer.getToContext()) BinaryOperator(
      LHS, RHS, E->getOpcode(), T, E->getValueKind(), E->getObjectKind(),
...
Expr *ASTNodeImporter::VisitConditionalOperator(ConditionalOperator *E) {
  ...
  return new (Importer.getToContext())
      ConditionalOperator(ToCond, Importer.Import(E->getQuestionLoc()), ToLHS,
                          Importer.Import(E->getColonLoc()), ToRHS, T,
...
ASTNodeImporter::VisitBinaryConditionalOperator(BinaryConditionalOperator *E) {
  ...
  OpaqueValueExpr *OpaqueValue =
      cast_or_null<OpaqueValueExpr>(Importer.Import(E->getOpaqueValue()));
  if (!OpaqueValue)
    return nullptr;
...
Expr *ASTNodeImporter::VisitOpaqueValueExpr(OpaqueValueExpr *E) {
  ...
  return new (Importer.getToContext())
      OpaqueValueExpr(Importer.Import(E->getLocation()), T, E->getValueKind(),
...
Expr *ASTNodeImporter::VisitArraySubscriptExpr(ArraySubscriptExpr *E) {
  ...
  return new (Importer.getToContext())
      ArraySubscriptExpr(ToLHS, ToRHS, T, E->getValueKind(), E->getObjectKind(),
...
Expr *ASTNodeImporter::VisitCompoundAssignOperator(CompoundAssignOperator *E) {
  ...
  return new (Importer.getToContext()) CompoundAssignOperator(
      LHS, RHS, E->getOpcode(), T, E->getValueKind(), E->getObjectKind(),
      CompLHSType, CompResultType, Importer.Import(E->getOperatorLoc()),
      E->isFPContractable());
}
...
Expr *ASTNodeImporter::VisitExplicitCastExpr(ExplicitCastExpr *E) {
  ...
  case Stmt::CStyleCastExprClass: {
    ...
    return CStyleCastExpr::Create(Importer.getToContext(), T, E->getValueKind(),
                                  E->getCastKind(), SubExpr, &BasePath, TInfo,
  ...
  case Stmt::CXXFunctionalCastExprClass: {
    ...
    return CXXFunctionalCastExpr::Create(
        Importer.getToContext(), T, E->getValueKind(), TInfo, E->getCastKind(),
        SubExpr, &BasePath, Importer.Import(FCE->getLParenLoc()),
  ...
  case Stmt::ObjCBridgedCastExprClass: {
    ...
    return new (Importer.getToContext()) ObjCBridgedCastExpr(
        ...
        E->getCastKind(), Importer.Import(OCE->getBridgeKeywordLoc()), TInfo,
        SubExpr);
  }
  default:
  ...
  case Stmt::CXXStaticCastExprClass:
    return CXXStaticCastExpr::Create(
        Importer.getToContext(), T, E->getValueKind(), E->getCastKind(),
        SubExpr, &BasePath, TInfo, ExprLoc, RParenLoc, Brackets);
...
Expr *ASTNodeImporter::VisitOffsetOfExpr(OffsetOfExpr *OE) {
  ...
  return OffsetOfExpr::Create(
      Importer.getToContext(), T, Importer.Import(OE->getOperatorLoc()), TInfo,
      Nodes, Exprs, Importer.Import(OE->getRParenLoc()));
}
...
Expr *ASTNodeImporter::VisitCXXNoexceptExpr(CXXNoexceptExpr *E) {
  ...
  return new (Importer.getToContext())
      CXXNoexceptExpr(T, Operand, CanThrow, Importer.Import(E->getLocStart()),
...
Expr *ASTNodeImporter::VisitCXXThrowExpr(CXXThrowExpr *E) {
  ...
  return new (Importer.getToContext())
      CXXThrowExpr(SubExpr, T, Importer.Import(E->getThrowLoc()),
                   E->isThrownVariableInScope());
}

Expr *ASTNodeImporter::VisitCXXDefaultArgExpr(CXXDefaultArgExpr *E) {
  ParmVarDecl *Param =
      cast_or_null<ParmVarDecl>(Importer.Import(E->getParam()));
  if (!Param)
    return nullptr;
...
Expr *ASTNodeImporter::VisitCXXScalarValueInitExpr(CXXScalarValueInitExpr *E) {
  ...
  return new (Importer.getToContext())
      CXXScalarValueInitExpr(T, TypeInfo, Importer.Import(E->getRParenLoc()));
}
...
Expr *ASTNodeImporter::VisitCXXBindTemporaryExpr(CXXBindTemporaryExpr *E) {
  ...
  auto *Dtor = cast_or_null<CXXDestructorDecl>(Importer.Import(
...
Expr *ASTNodeImporter::VisitCXXTemporaryObjectExpr(CXXTemporaryObjectExpr *CE) {
  ...
  auto *Ctor =
      cast_or_null<CXXConstructorDecl>(Importer.Import(CE->getConstructor()));
  if (!Ctor)
    return nullptr;

  return CXXTemporaryObjectExpr::Create(
      Importer.getToContext(), T, Importer.Import(CE->getLocStart()), Ctor,
...
ASTNodeImporter::VisitMaterializeTemporaryExpr(MaterializeTemporaryExpr *E) {
  ...
  auto *ToMTE = new (Importer.getToContext())
      MaterializeTemporaryExpr(T, TempE, E->isBoundToLvalueReference());
...
Expr *ASTNodeImporter::VisitCXXNewExpr(CXXNewExpr *CE) {
  ...
  FunctionDecl *OperatorNewDecl =
      cast_or_null<FunctionDecl>(Importer.Import(CE->getOperatorNew()));
  if (!OperatorNewDecl && CE->getOperatorNew())
    return nullptr;

  FunctionDecl *OperatorDeleteDecl =
      cast_or_null<FunctionDecl>(Importer.Import(CE->getOperatorDelete()));
  ...
  return new (Importer.getToContext())
      CXXNewExpr(Importer.getToContext(), CE->isGlobalNew(), OperatorNewDecl,
                 OperatorDeleteDecl, CE->passAlignment(),
                 CE->doesUsualArrayDeleteWantSize(), PlacementArgs,
                 Importer.Import(CE->getTypeIdParens()), ToArrSize,
                 CE->getInitializationStyle(), ToInit, T, TInfo,
...
Expr *ASTNodeImporter::VisitCXXDeleteExpr(CXXDeleteExpr *E) {
  ...
  FunctionDecl *OperatorDeleteDecl =
      cast_or_null<FunctionDecl>(Importer.Import(E->getOperatorDelete()));
...
Expr *ASTNodeImporter::VisitCXXConstructExpr(CXXConstructExpr *E) {
  ...
  CXXConstructorDecl *ToCCD = dyn_cast_or_null<CXXConstructorDecl>(
  ...
  return CXXConstructExpr::Create(
      Importer.getToContext(), T, Importer.Import(E->getLocation()), ToCCD,
...
Expr *ASTNodeImporter::VisitExprWithCleanups(ExprWithCleanups *EWC) {
  ...
  return ExprWithCleanups::Create(Importer.getToContext(), SubExpr,
                                  EWC->cleanupsHaveSideEffects(), Objs);
}
...
Expr *ASTNodeImporter::VisitCXXMemberCallExpr(CXXMemberCallExpr *E) {
  ...
  return new (Importer.getToContext())
      CXXMemberCallExpr(Importer.getToContext(), ToFn, ToArgs, T,
...
Expr *ASTNodeImporter::VisitMemberExpr(MemberExpr *E) {
  ...
  return MemberExpr::Create(Importer.getToContext(), ToBase, E->isArrow(),
                            ...
                            ToMember, ToFoundDecl, ToMemberNameInfo, nullptr, T,
                            E->getValueKind(), E->getObjectKind());
}
...
Expr *ASTNodeImporter::VisitInitListExpr(InitListExpr *ILE) {
  ...
  ASTContext &ToCtx = Importer.getToContext();
  InitListExpr *To =
      new (ToCtx) InitListExpr(ToCtx, Importer.Import(ILE->getLBraceLoc()),
  ...
  if (InitListExpr *SyntForm = ILE->getSyntacticForm()) {
    InitListExpr *ToSyntForm =
        cast_or_null<InitListExpr>(Importer.Import(SyntForm));
    if (!ToSyntForm)
      return nullptr;
...
Expr *ASTNodeImporter::VisitCXXDefaultInitExpr(CXXDefaultInitExpr *DIE) {
  FieldDecl *ToField =
      llvm::dyn_cast_or_null<FieldDecl>(Importer.Import(DIE->getField()));
...
Expr *ASTNodeImporter::VisitCXXNamedCastExpr(CXXNamedCastExpr *E) {
  ...
  if (isa<CXXStaticCastExpr>(E)) {
    return CXXStaticCastExpr::Create(Importer.getToContext(), ToType, VK, CK,
                                     ToOp, &BasePath, ToWritten, ToOperatorLoc,
                                     ToRParenLoc, ToAngleBrackets);
  } else if (isa<CXXDynamicCastExpr>(E)) {
    return CXXDynamicCastExpr::Create(Importer.getToContext(), ToType, VK, CK,
                                      ToOp, &BasePath, ToWritten, ToOperatorLoc,
                                      ToRParenLoc, ToAngleBrackets);
  } else if (isa<CXXReinterpretCastExpr>(E)) {
    return CXXReinterpretCastExpr::Create(
        Importer.getToContext(), ToType, VK, CK, ToOp, &BasePath, ToWritten,
...
DeclContext *ASTImporter::ImportContext(DeclContext *FromDC) {
  ...
  // When we're using a record/enum/Objective-C class/protocol as a context,
  // we
  ...
  if (RecordDecl *ToRecord = dyn_cast<RecordDecl>(ToDC)) {
    ...
    if (ToRecord->isCompleteDefinition()) {
      ...
      ASTNodeImporter(*this)
          .ImportDefinition(FromRecord, ToRecord, ASTNodeImporter::IDK_Basic);
    } else {
    ...
    if (ToEnum->isCompleteDefinition()) {
      ...
      ASTNodeImporter(*this)
          .ImportDefinition(FromEnum, ToEnum, ASTNodeImporter::IDK_Basic);
    } else {
      CompleteDecl(ToEnum);
    }
    ...
    if (ToClass->getDefinition()) {
      ...
      ASTNodeImporter(*this)
          .ImportDefinition(FromDef, ToClass, ASTNodeImporter::IDK_Basic);
    } else {
    ...
    if (ToProto->getDefinition()) {
      ...
      ASTNodeImporter(*this)
          .ImportDefinition(FromDef, ToProto, ASTNodeImporter::IDK_Basic);
    } else {
...
NestedNameSpecifier *ASTImporter::Import(NestedNameSpecifier *FromNNS) {
  ...
  case NestedNameSpecifier::TypeSpecWithTemplate: {
    ...
    if (!T.isNull()) {
      ...
      return NestedNameSpecifier::Create(ToContext, prefix, bTemplate,
                                         T.getTypePtr());
    }
  }
    return nullptr;
...
TemplateName ASTImporter::Import(TemplateName From) {
  ...
  case TemplateName::SubstTemplateTemplateParmPack: {
    ...
    TemplateTemplateParmDecl *Param = cast_or_null<TemplateTemplateParmDecl>(
...
SourceLocation ASTImporter::Import(SourceLocation FromLoc) {
  ...
  if (ToFileID.isInvalid())
    return SourceLocation();
  SourceLocation ret =
      ToSM.getLocForStartOfFile(ToFileID).getLocWithOffset(Decomposed.second);
  return ret;
}
...
CXXBaseSpecifier *ASTImporter::Import(const CXXBaseSpecifier *BaseSpec) {
  ...
  CXXBaseSpecifier *Imported = new (ToContext) CXXBaseSpecifier(
      Import(BaseSpec->getSourceRange()), BaseSpec->isVirtual(),
