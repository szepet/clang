  ...
  if (AMgr.options.shouldUnrollLoops()) {
    ...
    if (Term) {
      ...
      if (NewState != Pred->getState())
        nodeBuilder.generateNode(NewState, Pred);
      return;
    }
    auto UnrollingBlockVisitNum = getMaxBlockVisitOnPath(Pred->getState());
    if(UnrollingBlockVisitNum > 0)
      maxBlockVisitOnPath = UnrollingBlockVisitNum;
  }
